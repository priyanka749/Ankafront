import { MicrophoneIcon, PaperClipIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const Chatbot = () => {
  ...
  return (
    <div className="fixed bottom-0 right-0 w-[380px] h-[480px] bg-white border border-[#8B6B3E] shadow-xl rounded-2xl flex flex-col overflow-hidden z-50">
      {/* Header */}
      <div className="bg-[#8B6B3E] text-white px-4 py-3 flex items-center justify-between">
      ...
      {/* Listening indicator */}
      {listening && (
        <div className="text-center py-2 text-[#8B6B3E] text-xs font-medium animate-pulse bg-yellow-50 border-b border-yellow-200">
          🎤 Listening... Speak now
        </div>
      )}
        ...
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`px-4 py-3 text-sm rounded-xl max-w-[80%] shadow-md ${
              msg.sender === 'user'
                ? 'bg-[#8B6B3E] text-white rounded-br-none'
      ...
      <div className="border-t border-[#E8DCC3] px-3 py-3 bg-white flex items-center gap-2">
        ...
        <input
          ...
          className="flex-1 bg-[#f9f6f1] border border-[#8B6B3E] rounded-lg px-3 py-2 text-sm placeholder:text-[#8B6B3E]/60 text-[#2d2618] focus:outline-none focus:ring-2 focus:ring-[#8B6B3E] focus:border-transparent"
        />
        <button
          onClick={sendMessage}
          disabled={!input.trim()}
          className="bg-[#8B6B3E] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#a48454] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105"
        >
          Send
        </button>
      </div>
