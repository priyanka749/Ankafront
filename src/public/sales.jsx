...
<section className="w-full flex flex-col lg:flex-row items-stretch justify-between px-0 min-h-[500px] h-[400px] md:h-[500px] overflow-hidden bg-gradient-to-r from-[#fdf8f1] to-[#f0d8a8] rounded-2xl shadow-xl">
  <div className="w-full lg:w-1/2 flex flex-col justify-center items-center lg:items-start gap-6 max-w-3xl px-4 sm:px-8 lg:px-16 py-8">
    <div className="w-fit mb-4 animate-slide-up">
      <span className="block text-xl sm:text-2xl lg:text-3xl font-bold tracking-widest text-[#b89778] bg-[#fffaf0] border border-[#e6d3a3] px-8 py-3 rounded-lg uppercase shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-1" 
            style={{ letterSpacing: '0.15em', boxShadow: '0 6px 20px rgba(184,151,120,0.3)' }}>
        UP TO 60% OFF
      </span>
    </div>
    <h1 className="uppercase text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#5c4033] tracking-widest leading-tight animate-slide-up delay-100 drop-shadow-md">
      WINTER CLEARANCE
    </h1>
    <p className="text-[#6b5039] text-base sm:text-lg lg:text-xl max-w-lg text-center lg:text-left font-medium italic animate-slide-up delay-200">
      Elevate your wardrobe with exclusive deals on our premium collections.
    </p>
    <button className="bg-[#b89778] text-white font-semibold px-10 py-3.5 rounded-lg shadow-lg hover:bg-[#8c6b4b] transition-all duration-300 mt-4 text-lg tracking-wide animate-slide-up delay-300" 
            style={{ boxShadow: '0 8px 24px rgba(184,151,120,0.4)', borderRadius: '9999px' }}>
      Shop Now
    </button>
  </div>
  ...
  <div className="w-full lg:w-1/2 h-[300px] lg:h-full relative flex items-center justify-center">
    <img
      ...
      className="w-full h-full object-cover object-center border-l-4 border-[#D4A017] rounded-r-2xl shadow-2xl transition-opacity duration-500"
    ...
    <div className="absolute bottom-6 right-6 flex gap-3">
      {heroImages.map((_, index) => (
        <button
          key={index}
          className={`w-4 h-4 rounded-full ${index === currentHero ? 'bg-[#b89778] scale-110' : 'bg-[#d9c3a5]'} transition-all duration-300 border border-[#D4A017] hover:bg-[#a67c52]`}
