...
const ProductDetail = () => {
  ...
  if (loading) return <div className="text-center py-10 text-[#8B6B3E]">Loading product details...</div>;
  if (error) return <div className="text-center py-10 text-red-600">{error}</div>;
  if (!product) return null;

  return (
    <div className="bg-[#FFFBF7] text-[#8B6B3E] min-h-screen">
      <Navbar />
      <ToastContainer
        position="top-center"
        autoClose={2000}
      ...
      <div className="max-w-7xl mx-auto px-5 py-15 mt- grid grid-cols-1 md:grid-cols-2 gap-40 items-start">
          ...
          <div className="flex gap-4 flex-wrap justify-center">
            {thumbnails.map((img, index) => (
              <img
                ...
                className={`w-24 h-24 object-cover rounded-md cursor-pointer shadow-sm hover:ring-2 hover:ring-[#8B6B3E] ${
                  selectedImage === img ? 'ring-4 ring-[#8B6B3E]' : ''
                }`}
                alt={`thumb-${index}`}
              />
            ))}
        ...
        <div className="space-y-5">
                ...
                <button
                  ...
                  className={`px-4 py-2 border rounded-md text-sm ${
                    selectedSize === size
                      ...
                      : 'hover:bg-[#8B6B3E] hover:text-white border-[#ddd]'
                  }`}
                >
                  {size}
                </button>
          ...
          <div className="flex gap-6 mt-6 flex-wrap items-center">
            <button
              className="w-56 h-12 bg-[#8B6B3E] text-white rounded-full font-semibold text-lg flex items-center justify-center transition-all duration-200 hover:bg-[#6a5027]"
              onClick={() => handleAddToCart()}
            >
              Add To Cart
            </button>
            <button
              className="w-56 h-12 border-2 border-[#8B6B3E] text-[#8B6B3E] rounded-full font-semibold text-lg flex items-center justify-center transition-all duration-200 hover:bg-[#8B6B3E] hover:text-white bg-white"
              onClick={() => {
                const token = localStorage.getItem('token');
                if (!token) {
                  toast.error('Please log in to make a payment', {
          ...
          <button className="w-120 mt-6 bg-[#8B6B3E] hover:bg-[#6a5027] text-white py-3 text-lg font-medium rounded-lg">
            Virtual Try On
          </button>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 mt-10">
        ...
        <textarea
          ...
          className="w-full p-3 border border-[#8B6B3E] rounded-lg mb-3"
          rows={4}
        />
        <button
          onClick={handleReviewSubmit}
          className="bg-[#8B6B3E] text-white px-5 py-2 rounded hover:bg-[#704F2E]"
        >
          Submit Review
        </button>
      </div>
      ...
      {/* You May Also Like */}
      <section className="pt-12 pb-20 bg-[#FFFBF7] text-[#8B6B3E]">
        <h2 className="text-2xl sm:text-3xl font-extrabold mb-10 text-center">
          You May Also Like
          <span className="block w-24 h-1 bg-[#8B6B3E] mx-auto mt-2 rounded-full"></span>
        </h2>
          ...
          {relatedProducts.map((prod) => (
            ...
            >
              ...
              <div className="p-4 space-y-1">
                <h3 className="text-md font-semibold text-[#8B6B3E]">{prod.title}</h3>
                ...
                <div className="flex justify-between mt-3 gap-2">
                  <button
                    className="w-1/2 text-sm border border-[#8B6B3E] text-[#8B6B3E] rounded-full py-1 hover:bg-[#8B6B3E] hover:text-white"
                    onClick={() => navigate(`/product/${prod._id}`)}
                  >
                    Buy Now
                  </button>
                  <button
                    className="w-1/2 text-sm bg-[#8B6B3E] text-white rounded-full py-1 hover:bg-[#704F2E]"
